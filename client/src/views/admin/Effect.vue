<template>
  <el-row class="container">
    <el-row class="header">
      <div class="search">
        <el-select v-model="searchInfo" filterable placeholder="请选择" style="width: 300px;">
          <el-option v-for="item in effects" :key="item.id" :label="item.name" :value="item.name">
          </el-option>
        </el-select>
    ...
    <el-row class="main">
      <el-table :data="effects.slice((currentPage-1)*pagesize, currentPage*pagesize)" stripe style="width: 97%;">
      ...
      <el-col :span="24" style="text-align: center; margin-top: 10px;">
        <el-pagination layout="prev, pager, next" :page-size="pagesize" @current-change="current_change" 
          :current-page.sync="currentPage" :pager-count="5" :total="effects.length">
        </el-pagination>
      </el-col>
    </el-row>
    <!-- 模态框 -->
...
export default {
  ...
  methods: {
    ...
    search(){
      this.temp = this.effects;
      this.effects = _.filter(this.effects, { name: this.searchInfo });
    },
    cancel(){
      this.effects = this.temp;
      this.searchInfo = '';
    },
    // 提交点击事件
    submit(){
